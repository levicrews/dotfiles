#+title: Bootstrap
#+author: Levi Crews
#+email: levigcrews@gmail.com

* TODO add file-local-var to automatically tangle this doc
* TODO add description of this file
* bootstrap script

Adapted from https://github.com/rkalis/dotfiles and associated blog posts.

** setup

#+begin_src bash :tangle "bootstrap"
#!/usr/bin/env bash
set -euo pipefail
cd "$(dirname "$0")"

echo "[sudo] acquiring credentials..."
sudo -v

# --- One-time repo setup (idempotent) --- #
sudo apt-get update -y
sudo apt-get install -y software-properties-common curl wget gpg
sudo add-apt-repository -y ppa:inkscape.dev/stable || true
sudo add-apt-repository -y ppa:tomtomtom/yt-dlp || true
# Microsoft VS Code apt repo (minimal + idempotent)
if ! grep -qr 'https://packages.microsoft.com/repos/code' /etc/apt/sources.list* /etc/apt/sources.list.d 2>/dev/null; then
    sudo install -d -m 755 /usr/share/keyrings
    wget -qO- https://packages.microsoft.com/keys/microsoft.asc | sudo gpg --dearmor --yes -o /usr/share/keyrings/packages.microsoft.gpg
    echo "deb [arch=amd64 signed-by=/usr/share/keyrings/packages.microsoft.gpg] https://packages.microsoft.com/repos/code stable main" \
        | sudo tee /etc/apt/sources.list.d/vscode.list >/dev/null
    sudo apt-get update -y
fi
sudo apt-get update -y

# --- Rust only if needed (cargo.list present) --- #
if [[ -f cargo.list ]] && ! command -v cargo >/dev/null 2>&1; then
    echo "Installing Rust toolchain (rustup) because cargo.list is present..."
    curl -fsSL https://sh.rustup.rs | sh -s -- -y
fi
export PATH="$HOME/.cargo/bin:$PATH"

#+end_src

** install packages from managers

#+begin_src bash :tangle "bootstrap"
# --- Helpers --- #
lines() { grep -vE '^\s*(#|$)' "$1" || true; }	# skip comments/blank lines

apt_installed()   { dpkg-query -W -f='${Status}\n' "$1" 2>/dev/null | grep -q "install ok installed"; }
snap_installed()  { snap list "$1" >/dev/null 2>&1; }
npm_installed()   { npm list -g --depth=0 "$1" >/dev/null 2>&1; }
gem_installed()   { gem list -i "$1" >/dev/null 2>&1 }
cargo_installed() { cargo install --list 2>/dev/null | awk '{print $1}' | grep -xq "$1"; }

# VS Code extension helpers: normalize ID and check installed
code_id() { printf '%s' "$1" | sed -E 's/^[[:space:]]+|[[:space:]]+$//g;s/[[:space:]]+//g' | awk -F'@' '{print tolower($1)}'; }
code_installed() {
    local id; id="$(code_id "$1")"
    code --list-extensions 2>/dev/null | tr '[:upper:]' '[:lower:]' | grep -xq "$id"
}

ensure_tool() {
    case "$1" in
        code)
            command -v code >/dev/null 2>&1 || sudo apt-get install -y code
            ;;
        npm)
            command -v npm  >/dev/null 2>&1 || sudo apt-get install -y nodejs npm
            ;;
        uvtool|uv)
            # We use uv for Python CLIs; make sure prerequisites are there.
            command -v python3 >/dev/null 2>&1 || sudo apt-get install -y python3 python3-venv
            command -v uv >/dev/null 2>&1 || curl -LsSf https://astral.sh/uv/install.sh | sh
            ;;
    esac
}

install_pkg() {
    local tool="$1" line="$2"
    local name="${line%% *}"	# first token (handles flags like "--classic")

    case "$tool" in
        apt|apt-get)
            apt_installed "$name" || sudo apt-get install -y $line
            ;;
        snap)
            snap_installed "$name" || sudo snap install $line
            ;;
        npm)
            npm_installed "$name" || sudo npm install -g $line
            ;;
        gem)
            gem_installed "$name" || sudo gem install $line
            ;;
        uvtool|uv)
            # uv is idempotent; re-running is safe & quiet if already installed.
			uv tool install "$line" || echo "(warn) uv failed to install: $line"
			;;
        cargo)
            cargo_installed "$name" || cargo install $line
            ;;
        code)
            # Idempotent: skip if already installed (tolerates @version / whitespace)
            if ! code_installed "$line"; then
                # Try exactly as listed first (allows @version), then fall back to bare ID
                code --install-extension "$line" \
                || code --install-extension "$(code_id "$line")" \
                || echo "	(warn) failed to install VS Code extension: $line"
            fi
            ;;
        *)
            : ;;  # unknown tool: skip
    esac
}

# --- Main loop --- #
shopt -s nullglob
for f in *.list; do
    case "$(basename -- "$f")" in
        gogh.list|pylib.list) continue ;;
    esac

    tool="${f%.*}"	# e.g., apt.list -> apt
    echo "==> Installing $tool packages..."
    ensure_tool "$tool"

    while IFS= read -r pkg; do
        # trim whitespace; skip empty after trimming
        pkg="$(printf '%s' "$pkg" | sed -E 's/^[[:space:]]+|[[:space:]]+$//g')"
        [[ -z "$pkg" ]] && continue
        install_pkg "$tool" "$pkg"
    done < <(lines "$f")

    echo "<== Finished installing $tool packages."
done

#+end_src

** install packages from source

#+begin_src bash :tangle "bootstrap"
# --- Install from source --- #
echo "==> Installing packages from source..."

# 1) Starship prompt (install only if missing)
if ! command -v starship >/dev/null 2>&1; then
    curl -sS https://starship.rs/install.sh | sh -s -- -y
fi

# 2) Gogh GNOME Terminal themes driven by gogh.list (install only if missing)
if command -v gnome-terminal >/dev/null 2>&1 && command -v gsettings >/dev/null 2>&1; then
    export TERMINAL=gnome-terminal

    # does a GNOME Terminal profile with visible-name == $1 exist?
    gnome_profile_exists() {
        local want="$1"
        local uuids
        uuids="$(gsettings get org.gnome.Terminal.ProfilesList list 2>/dev/null | tr -d "[],'")" || return 1
        for uuid in $uuids; do
            local vis
            vis="$(gsettings get "org.gnome.Terminal.Legacy.Profile:/org/gnome/terminal/legacy/profiles:/:${uuid}/" visible-name 2>/dev/null || true)"
            vis="${vis//\'}"
            if [[ "$vis" == "$want" ]]; then return 0; fi
        done
        return 1
    }

    # ensure repo present/updated
    if [[ -d "$HOME/util/gogh/.git" ]]; then
        git -C "$HOME/util/gogh" fetch --all -p >/dev/null 2>&1 || true
        git -C "$HOME/util/gogh" pull --ff-only >/dev/null 2>&1 || true
    else
        git clone https://github.com/levicrews/Gogh.git "$HOME/util/gogh" # use my fork with extra themes
    fi

    # permanently normalize default profile name (Gogh expects "Default")
    default_uuid="$(gsettings get org.gnome.Terminal.ProfilesList default | tr -d "'")"
    curr_name="$(gsettings get "org.gnome.Terminal.Legacy.Profile:/org/gnome/terminal/legacy/profiles:/:${default_uuid}/" visible-name | tr -d "'")"
    if [[ "$curr_name" != "Default" ]]; then
        gsettings set "org.gnome.Terminal.Legacy.Profile:/org/gnome/terminal/legacy/profiles:/:${default_uuid}/" visible-name "Default"
    fi

    # read themes from gogh.list (one per line, .sh optional); else default to "atom dracula"
    if [[ -f gogh.list ]]; then
        themes="$(grep -vE '^\s*(#|$)' gogh.list | tr '\n' ' ')"
    fi

    pushd "$HOME/util/gogh/installs" >/dev/null
    for script in $themes; do
        slug="${script%.sh}"
        theme_name="${slug^}"	# "atom" -> "Atom"
        if ! gnome_profile_exists "$theme_name"; then
            "./${slug}.sh" || true	# don't abort entire bootstrap if a single theme fails
        fi
    done
    popd >/dev/null
fi

# 3) Sparklines in terminal (install only if missing)
if ! command -v spark >/dev/null 2>&1; then
    sudo sh -c "curl https://raw.githubusercontent.com/holman/spark/master/spark -o /usr/local/bin/spark && chmod +x /usr/local/bin/spark"
fi

# 4) cheat.sh client
if ! command -v cht.sh >/dev/null 2>&1; then
    curl -s https://cht.sh/:cht.sh | sudo tee /usr/local/bin/cht.sh && sudo chmod +x /usr/local/bin/cht.sh
fi

echo "<== Finished installing packages from source."

#+end_src

*** [[https://github.com/starship/starship][Starship]]: the minimal, blazing-fast, and infinitely customizable prompt for any shell

- installed with ~curl~ above
- ~eval~'ed in =~/.bashrc=
- configuration in =~/.config/starship.toml=
- cf. [[https://blog.meain.io/2022/my-shell-prompt/][meain]] :: I like its use of both the left and right margins

*** [[https://github.com/Gogh-Co/Gogh][Gogh]]: color schemes for Gnome Terminal

prerequisites (in =apt.list=): ~dconf-cli~, ~uuid-runtime~

** install fonts

Microsoft [[https://github.com/microsoft/cascadia-code][Cascadia Code]] + GitHub [[https://github.com/githubnext/monaspace][Monaspace]]

#+begin_src bash :tangle "bootstrap"
# --- Install fonts --- #
echo "==> Installing fonts..."

# 1) Base Cascadia Code via apt (skip if any Cascadia Code already installed)
if ! fc-list | grep -qi 'Cascadia Code'; then
    if ! sudo apt-get install -y fonts-cascadia-code; then
        echo "  (info) apt package not available/succeeded; will rely on upstream zip"
    fi
fi

# 2) Ensure Nerd Font–patched variants (incl. italics) are installed
#    Look for Microsoft's NF naming (e.g., “Cascadia Code NF”, “Cascadia Mono NF”).
if ! fc-list | grep -qiE 'Cascadia.*Nerd|Nerd Font.*Cascadia|Cascadia .* NF'; then
    tmp="$(mktemp -d)"
    zip_url="https://github.com/microsoft/cascadia-code/releases/latest/download/CascadiaCode-ttf.zip"
    echo "  (info) Nerd Font variants not found; fetching from upstream…"
    if curl -fsSL -o "$tmp/cc.zip" "$zip_url"; then
        mkdir -p "$HOME/.local/share/fonts/CascadiaCode"
        unzip -qo "$tmp/cc.zip" -d "$tmp/cc" >/dev/null
        # Copy *all* TTFs from the release (covers Code/Mono, italics, PL, NF)
        find "$tmp/cc" -type f -iname "*.ttf" -exec cp -f {} "$HOME/.local/share/fonts/CascadiaCode/" \;
        fc-cache -f >/dev/null
        echo "  (ok) Installed Cascadia Code (including Nerd Font & italics) into ~/.local/share/fonts/CascadiaCode"
    else
        echo "  (warn) could not fetch Cascadia Code from GitHub; skipping NF install"
    fi
    rm -rf "$tmp"
else
    echo "  (ok) Cascadia Nerd Font variants already present"
fi

# 3) Monaspace (clone repo once; copy fonts if missing)
if ! fc-list | grep -qi 'Monaspace'; then
    mkdir -p "$HOME/.local/share/fonts/Monaspace"
    if [[ -d "$HOME/util/monaspace/.git" ]]; then
        git -C "$HOME/util/monaspace" pull --ff-only >/dev/null 2>&1 || true
    else
        git clone https://github.com/githubnext/monaspace "$HOME/util/monaspace"
    fi
    find "$HOME/util/monaspace" -type f \( -name "*.otf" -o -name "*.ttf" \) -exec cp -u {} "$HOME/.local/share/fonts/Monaspace/" \;
    fc-cache -f >/dev/null
fi

echo "<== Finished installing fonts."

#+end_src

** install runtimes

#+begin_src bash :tangle "bootstrap"
# --- Runtimes (Julia via juliaup) --- #
echo "==> Installing runtimes..."
if ! command -v juliaup >/dev/null 2>&1; then
    curl -fsSL https://install.julialang.org | sh -s -- -y
fi
echo "<== Finished installing runtimes."

#+end_src

** GNOME settings

#+begin_src bash :tangle "bootstrap"
# --- Remap Caps Lock to Ctrl --- #
echo "==> Remapping keys..."
gsettings set org.gnome.desktop.input-sources xkb-options "['caps:ctrl_modifier']"
echo "<== Finished remapping keys."

# --- Set dark system theme --- #
echo "==> Setting dark system theme..."
gsettings set org.gnome.desktop.interface gtk-theme 'Yaru-dark'
echo "<== Finished setting dark system theme."

#+end_src

** initialize Git globals

#+begin_src bash :tangle "bootstrap"
# --- Initialize Git (globals, LFS, Delta diffs) --- #
echo "==> Initializing Git..."
git config --global fetch.prune true
git config --global status.short true   # like: git status -s
git config --global status.branch true  # add branch/upstream line (like -b)
git config --global core.fileMode false # ignore file permissions

if command -v git-lfs >/dev/null 2>&1; then
    git lfs install --skip-repo >/dev/null 2>&1 || git lfs install >/dev/null 2>&1
fi

# Set delta as Git pager only if delta exists AND no pager is configured yet
if command -v delta >/dev/null 2>&1 && ! git config --global --get core.pager >/dev/null 2>&1; then
    git config --global core.pager delta
    git config --global delta.side-by-side true
    git config --global interactive.diffFilter 'delta --color-only'
    git config --global delta.navigate true # use n and N to move between diff sections
    git config --global merge.conflictStyle zdiff3
fi
echo "<== Finished initializing Git."

#+end_src

** end

#+begin_src bash :tangle "bootstrap"
# --- END --- #
echo "Bootstrap complete."
#+end_src

* lists
** apt.list
*** list
#+begin_src :tangle "apt.list"
autojump
batcat
bmon
cmake
dconf-cli
dwdiff
emacs
fd-find
font-manager
fonts-powerline
fzf
gdal-bin
gdebi
gdebi-core
gh
git-delta
git-lfs
gnome-shell-extension-prefs
golang-go
gnome-sushi
graphviz
htop
httpie
hugo
icdiff
inkscape
jq
just
manpages-dev
mdp
miller
mmv
moreutils
neofetch
nodejs
obs-studio
pandoc
papirus-icon-theme
pdfgrep
pdftk-java
poppler-utils
pympress
python3-pyqt5
qalc
qpdfview
r-base
ripgrep
rlwrap
rofi
ruby-full
samba
scrot
silversearcher-ag
shellcheck
solaar
sqlite3
surfraw
sword-text-kjv
sxhkd
texlive-latex-extra
texlive-fonts-extra
texlive-extra-utils
texlive-science
latexmk
tmux
uuid-runtime
viewnior
visidata
vlc
w3m
wordnet
xclip
yadm
yt-dlp
zathura
zoxide
#+end_src
*** notes

A number of the tools I install are modern/faster/saner alternatives to common Unix commands. A collection of such tools is maintained in the [[https://github.com/ibraheemdev/modern-unix][modern-unix]] repo on GitHub. See also [[https://jvns.ca/blog/2022/04/12/a-list-of-new-ish--command-line-tools/][Julia Evans's list]].

**** [[https://github.com/wting/autojump][autojump]]: A =cd= command that learns (Python)
**** [[https://github.com/ajeetdsouza/zoxide][zoxide]]: a smarter =cd= command (Rust)
**** [[https://github.com/sharkdp/bat][bat]]: =cat= clone with syntax highlighting and Git integration
- aliased for =cat=
**** [[https://github.com/tgraf/bmon][bmon]]: bandwidth monitor
- called by ~rofi~
**** [[https://github.com/junegunn/fzf][fzf]]: a command line fuzzy finder
- tips from [[https://macowners.club/posts/favorite-apps-tools-4/][Mac Owners Club]]
**** [[https://github.com/cli/cli][gh]]: GitHub CLI
- supercedes HTTPS and SSH interactions with GitHub
**** [[https://httpie.io/][httpie]]: user-friendly command-line HTTP client for the API era
- comes with JSON support, syntax highlighting, persistent sessions, wget-like downloads, plugins, ...
- can replace =curl= and =wget=
**** [[https://github.com/inkscape/inkscape][inkscape]]: free and open source vector drawing
- Castel: [[https://castel.dev/post/lecture-notes-2/][How I draw figures for my math notes using Inkscape]]
**** [[https://github.com/casey/just][just]]: just a command runner
**** [[https://stedolan.github.io/jq/tutorial/][jq]]: a lightweight and flexible command-line JSON processor
**** [[https://github.com/visit1985/mdp][mdp]]: command-line based Markdown presentation tool
**** [[https://github.com/johnkerl/miller][miller]]: like =awk=, =sed=, =cut=, =join=, and =sort= for name-indexed data such as CSV, TSV, and tabular JSON
**** [[https://github.com/itchyny/mmv][mmv]]: rename multiple files using your default editor
**** [[https://github.com/dylanaraps/neofetch][neofetch]]: a command line system information tool
- see [[https://neil.computer/notes/neofetch/][this configuration]] from [[file:../../Dropbox/org/roam/refs/berkeley_graphics.org][Berkeley Graphics]]
**** pdftk: command line tool to merge and split PDFs
- used to compile class slides into one PDF for entry in BibTeX database
- synatx :: ~pdftk <files> cat output <newfile>~
**** qalc: powerful and easy to use command line calculator
- called by ~rofi~ launcher
**** pdf viewers: qpdfview & zathura
**** [[https://github.com/BurntSushi/ripgrep][ripgrep]]: =grep=, but better
**** rofi
- explained in detail elsewhere
**** [[https://github.com/dreamer/scrot][scrot]]: command-line screenshot utility
- called by ~rofi~
**** [[https://github.com/ggreer/the_silver_searcher][silversearcher-ag]]: A code-searching tool similar to ack, but faster
**** [[https://pwr-solaar.github.io/Solaar/][solaar]]: Linux manager for Logitech Unifying Receiver devices
**** [[https://gitlab.com/surfraw/Surfraw][surfraw]]: CLI to popular web search engines
**** sxhkd
- hotkey manager used to specify ~rofi~ hotkeys
- explained in detail elsewhere
**** texlive
- a [[https://tex.stackexchange.com/questions/245982/differences-between-texlive-packages-in-linux][description]] of the texlive package options
**** [[https://www.visidata.org/][visidata]]: interactive multitool for tabular data
**** [[https://github.com/yt-dlp/yt-dlp][yt-dlp]]: download YouTube videos
*** dependencies for =diff-pdf=
#+begin_src
automake
g++
libpoppler-glib-dev
libwxgtk3.0-dev
poppler-utils
#+end_src
** snap.list
*** list
#+begin_src :tangle "snap.list"
spotify
jabref
#+end_src
** uvtool.list
*** list
#+begin_src :tangle "uvtool.list"
eg
gdown
grip
radian
ruff
svg2tikz
tldr
pyomo
#+end_src
*** notes
**** [[https://github.com/srsudar/eg][eg]]: useful examples at the command line
**** [[https://github.com/wkentaro/gdown][gdown]]: Download a large file from Google Drive
~curl~ and ~wget~ fail because Google gives a security notice that they can't bypass automatically.
**** [[https://github.com/joeyespo/grip][grip]]: Preview GitHub READMEs locally
**** [[https://github.com/randy3k/radian][radian]]: A 21st-century R console
**** [[https://github.com/astral-sh/ruff][ruff]]: An extremely fast Python linter, written in Rust
**** [[https://github.com/tldr-pages/tldr][tldr]]: Community-sourced command-line cheatsheets
**** [[https://github.com/tqdm/tqdm][tqdm]]: a fast, extensible progress bar for Python and CLI
**** [[http://www.pyomo.org/about][pyomo]]: modeling language with a diverse set of optimization capabilities
** pylib.list

The script does not automatically install these libraries. This list is for my reference when setting up Python environments.

*** list
#+begin_src :tangle "pylib.list"
bokeh
dask
dataprep
docutils
econtools
gme
lux-api
minet
pandas-datareader
pandas-profiling[notebook]
pyjanitor
pyreadr
quantecon
wrds
#+end_src
*** notes
**** [[https://github.com/bokeh/bokeh][bokeh]]: interactive visualization library for modern web browsers
It provides elegant, concise construction of versatile graphics, and affords high-performance interactivity over large or streaming datasets. Bokeh can help anyone who would like to quickly and easily make interactive plots, dashboards, and data applications.
**** [[https://dask.org/][dask]]: scaling and parallelizing Pandas
**** [[https://github.com/sfu-db/dataprep][dataprep]]: The easiest way to prepare data in Python
**** [[https://pypi.org/project/docutils/][docutils]]: convert reStructuredText to HTML and LaTeX
**** [[http://www.danielmsullivan.com/econtools/][econtools]]: package with tools for econometrics and data munging
**** [[https://www.usitc.gov/data/gravity/gme.htm][gme]]: Python module for USITC Gravity Modeling Environment
**** [[https://github.com/medialab/minet][minet]]: webmining command line tool
- can be used to collect and extract data from a large variety of web sources such as raw webpages, Facebook, YouTube, Twitter, etc.
**** [[https://pydata.github.io/pandas-datareader/index.html][pandas-datareader]]: remote data access for pandas
**** [[https://github.com/pandas-profiling/pandas-profiling][pandas-profiling]]: Create HTML profiling reports from pandas DataFrame objects
**** [[https://github.com/pyjanitor-devs/pyjanitor][pyjanitor]]: Clean APIs for data cleaning (Python implementation of R package Janitor)
**** [[https://github.com/ofajardo/pyreadr][pyreadr]]: Python package to read/write R data files
**** [[https://github.com/wharton/wrds][wrds]]: extract [[file:../../Dropbox/org/roam/refs/wrds.org][WRDS]] data into Pandas
** cargo.list
*** list
#+begin_src :tangle "cargo.list"
broot
du-dust
eza
sd
#+end_src
*** notes
**** [[https://github.com/Canop/broot][broot]]: a new way to see and navigate directory trees
**** [[https://github.com/bootandy/dust][dust]]: a more intuitive version of =du= (disk usage) in rust
**** [[https://github.com/eza-community/eza][eza]]: a modern replacement for =ls=
**** [[https://github.com/chmln/sd][sd]]: intuitive find & replace CLI (=sed= alternative)
** code.list
Check for updates to this list with ~code --list-extensions~
#+begin_src :tangle "code.list"
aaron-bond.better-comments
affenwiesel.matlab-formatter
akamud.vscode-theme-onedark
albert.tabout
alefragnani.numbered-bookmarks
alefragnani.project-manager
alefragnani.rtf
arcticicestudio.nord-visual-studio-code
axetroy.vscode-changelog-generator
bazelbuild.vscode-bazel
bierner.emojisense
bierner.markdown-checkbox
brunnerh.insert-unicode
charliermarsh.ruff
chouzz.vscode-better-align
codespaces-contrib.codeswing
davidanson.vscode-markdownlint
dlasagno.rasi
docker.docker
donjayamanne.githistory
eamodio.gitlens
ericadamski.carbon-now-sh
formulahendry.code-runner
geddski.macros
gimly81.matlab
github.copilot
github.copilot-chat
github.remotehub
github.vscode-pull-request-github
grapecity.gc-excelviewer
guidotapia2.unicode-math-vscode
imjacobclark.vscode-lisp-formatter
james-yu.latex-workshop
julialang.language-julia
kevinrose.vsc-python-indent
kokakiwi.vscode-just
kylebutts.vscode-stata
matklad.pale-fire
mattn.lisp
mechatroner.rainbow-csv
mikestead.dotenv
mosbasik.sxhkdrc-syntax
ms-azuretools.vscode-containers
ms-azuretools.vscode-docker
ms-python.debugpy
ms-python.isort
ms-python.python
ms-python.vscode-pylance
ms-python.vscode-python-envs
ms-toolsai.datawrangler
ms-toolsai.jupyter
ms-toolsai.jupyter-keymap
ms-toolsai.jupyter-renderers
ms-toolsai.vscode-jupyter-cell-tags
ms-toolsai.vscode-jupyter-slideshow
ms-vscode-remote.remote-containers
ms-vscode-remote.remote-ssh
ms-vscode-remote.remote-ssh-edit
ms-vscode.azure-repos
ms-vscode.cmake-tools
ms-vscode.cpptools
ms-vscode.cpptools-extension-pack
ms-vscode.cpptools-themes
ms-vscode.makefile-tools
ms-vscode.remote-explorer
ms-vscode.remote-repositories
ms-vscode.vscode-github-issue-notebooks
ms-vsliveshare.vsliveshare
mxschmitt.postscript
oijaz.unicode-latex
openai.chatgpt
orangex4.latex-sympy-calculator
patbenatar.advanced-new-file
phr0s.bib
pkief.material-icon-theme
quicktype.quicktype
randomfractalsinc.geo-data-viewer
redhat.vscode-yaml
reditorsupport.r
reditorsupport.r-syntax
shd101wyy.markdown-preview-enhanced
sidneys1.gitconfig
sleistner.vscode-fileutils
stkb.rewrap
tamasfe.even-better-toml
tecosaur.latex-utilities
tomoki1207.pdf
tootone.org-mode
tyriar.vscode-terminal-here
visualstudioexptteam.intellicode-api-usage-examples
visualstudioexptteam.vscodeintellicode
vivaxy.vscode-conventional-commits
vmsynkov.colonize
vscode-icons-team.vscode-icons
vscode-org-mode.org-mode
vsls-contrib.codetour
vsls-contrib.gistfs
wmaurer.change-case
wmaurer.vscode-jumpy
xshrim.txt-syntax
yeaoh.statarun
yzane.markdown-pdf
yzhang.markdown-all-in-one
#+end_src
** npm.list
*** list
#+begin_src :tangle "npm.list"
@bazel/bazelisk
@openai/codex
opencode-ai@latest
carbon-now-cli
how-2
tldr
decktape
#+end_src
*** notes
**** [[file:~/Dropbox/org/roam/refs/bazel.org][bazel]]: Build and test software of any size, quickly and reliably
**** [[https://developers.openai.com/codex/][codex]]: OpenAI's coding agent
**** [[https://github.com/anomalyco/opencode][opencode]]: The open source coding agent
**** [[https://github.com/carbon-app/carbon][carbon]] and [[https://github.com/mixn/carbon-now-cli][carbon-now-cli]]: Create and share beautiful images of your source code
**** [[https://github.com/santinic/how2][how2]]: Stackoverflow from the terminal
**** [[https://github.com/tldr-pages/tldr][tldr]]: Collaborative cheatsheets for console commands
**** [[https://github.com/astefanutti/decktape][decktape]]: PDF exporter for HTML presentations
** gem.list
*** list
#+begin_src :tangle "gem.list"
anystyle-cli
#+end_src
*** notes
**** [[https://github.com/inukshuk/anystyle][anystyle]]: ML-based bib parser used by [[https://github.com/org-roam/org-roam-bibtex/blob/main/doc/orb-manual.org#orb-pdf-scrapper---retrieve-references-from-pdfs][Orb PDF Scrapper]]
** gogh.list
*** list
#+begin_src :tangle "gogh.list"
afterglow
andromeda
arthur
chalk
dot-gov
github-dark
lab-fox
liquid-carbon
neutron
nord
sea-shells
spacedust
zenburn
#+end_src
*** notes
* custom bash scripts

** ~pdfsearch~: search all PDFs in current directory for string

[[https://stackoverflow.com/a/4643518/11952647][This Stackoverflow answer]] suggests the =find ... -exec sh -c ...=
framework that comprises the body of this short script. [[https://stackoverflow.com/a/4643518/11952647][This answer]]
taught me how to pass the search query to the =-exec= command.
The =-q= flag for =pdftotext= just silences error messages.
The =-m 1= flag for =grep= limits results to one per file.

#+begin_src shell :tangle "/usr/local/bin"
#! /usr/bin/env sh

tosearch=$1
find . -name '*.pdf' -exec sh -c 'pdftotext "{}" -q - | grep -m 1 --with-filename --label="{}" --color "$1"' sh "$tosearch" \;
#+end_src

** [[https://github.com/cheat/cheat][cheat]]: create and view custom interactive cheatsheets on the command line

- Cheatsheets are stored on "cheatpaths", which are directories that contain cheatsheets. Cheatpaths are specified in a YAML config file.
- Can store personal cheatsheets *and* community cheatsheets
- compare [[https://github.com/srsudar/eg][eg]] (in ~pip.list~) and [[https://github.com/tldr-pages/tldr][tldr]] (in ~npm.list~), which *only* provide stock examples
- compare [[https://github.com/gnebbia/kb][kb]], which aims to be a full CLI knowledge base
