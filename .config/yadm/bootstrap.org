...
#+begin_src shell :tangle "bootstrap"
#!/usr/bin/env bash
set -euo pipefail

# Run from the dotfiles dir (where the *.list files are)
cd "$(dirname "$0")"

echo "[sudo] acquiring credentials..."
sudo -v

# Minimal prereqs, then PPAs, then update
...
# Rust (non-interactive) + PATH for this session
if ! command -v cargo >/dev/null 2>&1; then
  curl -fsSL https://sh.rustup.rs | sh -s -- -y
fi
export PATH="$HOME/.cargo/bin:$PATH"

# Helper: stream non-empty, non-comment lines
...
for f in *.list; do
  ...
  while IFS= read -r pkg; do
    case "$tool" in
      ...
      code)        command -v code >/dev/null && code --install-extension "$pkg" ;;
    esac
  done < <(lines "$f")
...
Taken again from https://github.com/rkalis/dotfiles and associated blog posts. Called in ~bootstrap~ to give the user easy-to-read progress updates as the installation proceeds.
